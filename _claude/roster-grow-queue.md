...
> **đúng 1 mục** còn `[pending]` VÀ đã có URL nguồn (theo thứ tự từ trên
> xuống), gọi `roster_grow_worker.py --queue-item "<mô tả>" --source-urls
> "<url 1>" "<url 2>"...`, merge kết quả đã qua kiểm vào `ROSTER`, rồi đánh
> dấu mục đó `[done]` kèm ngày + số mục thêm được ngay tại dòng đó (ghi đè,
> không xoá mục cũ — giữ lại làm nhật ký tiến độ).
>
> **Mỗi mục cần 1-3 URL nguồn cụ thể** (trang danh bạ hiệp hội, trang
> Wikipedia liệt kê, trang chính phủ/đại học...) — `url_context` của Gemini
> chỉ đọc được URL được giao sẵn, không tự tìm kiếm mở. Mục chưa có URL (ghi
> `URL: — (cần bổ sung)`) thì bỏ qua, coi như chưa `[pending]` thật sự.
>
> Hết hàng đợi (không còn mục `[pending]` có URL) → hôm đó không làm gì,
> không phải lỗi. Sếp bổ sung thêm mục/URL vào file khi cần.
>
...
1. [pending] Malaysia (hiện có 4) — TTO/trung tâm ĐMST các đại học công lập lớn ngoài UM/USM/UKM/UTM đã có
   - URL: https://itma.my/tech-transfer-in-malaysia/ (đã kiểm chứng 2026-09-07: trang liệt kê ~22 đơn vị, không có link riêng từng đơn vị nên nhiều URL Gemini ước lượng sẽ sai — bước kiểm tra sống của worker sẽ tự lọc)
